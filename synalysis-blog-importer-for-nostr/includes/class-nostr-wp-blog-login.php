...
final class Nostr_WP_Blog_Login {
	...
	public const REST_NAMESPACE = 'synalysis-blog-importer-for-nostr/v1';
	...
	public function register_rest_routes(): void {
		register_rest_route(
			...
			array(
				...
				'permission_callback' => array( $this, 'rest_permission_login_challenge_public' ),
			)
		);
		register_rest_route(
			self::REST_NAMESPACE,
			...
			array(
				...
				'permission_callback' => array( $this, 'rest_permission_login_verify_public' ),
			)
		);
	}

	/**
	 * Intentionally public: unauthenticated users on wp-login.php request a NIP-07 challenge (nonce + relay tag).
	 */
	public function rest_permission_login_challenge_public(): bool {
		return true;
	}

	/**
	 * Intentionally public: verifies a signed NIP-42-style event then sets the auth cookie (no prior session).
	 */
	public function rest_permission_login_verify_public(): bool {
		return true;
	}
	...
	public function login_enqueue_scripts(): void {
		 ...
		 * the property is still undefined) and dispatch synalysis-blog-importer-for-nostr:provider.
		 */
		wp_register_script(
			'synalysis-blog-importer-login-boot',
			false,
			array(),
			NOSTR_WP_BLOG_VERSION,
			true
		);
		wp_enqueue_script( 'synalysis-blog-importer-login-boot' );
		...
		wp_enqueue_script(
			'synalysis-blog-importer-login',
			NOSTR_WP_BLOG_URL . 'assets/js/nostr-login.js',
			array( 'synalysis-blog-importer-login-boot' ),
			NOSTR_WP_BLOG_VERSION,
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'redirectTo' => $redirect,
				'strings'    => array(
					'button'    => __( 'Sign in with browser extension', 'synalysis-blog-importer-for-nostr' ),
					'noExtension' => __( 'No compatible browser extension found. Install one that supports NIP-07 and try again.', 'synalysis-blog-importer-for-nostr' ),
					'working'   => __( 'Waiting for signature…', 'synalysis-blog-importer-for-nostr' ),
					'failed'    => __( 'Extension sign-in failed.', 'synalysis-blog-importer-for-nostr' ),
					'notAuthor' => __( 'This key is not listed as an author for this site.', 'synalysis-blog-importer-for-nostr' ),
				),
			)
		);
	}
	...
	public function login_form(): void {
		...
		?>
		<div id="synalysis-blog-importer-login" class="synalysis-blog-importer-login" style="margin-top:1em;padding-top:1em;border-top:1px solid #c3c4c7;">
			<p>
				<button type="button" class="button button-secondary" id="synalysis-blog-importer-login-btn" disabled>
					<?php esc_html_e( 'Sign in with browser extension', 'synalysis-blog-importer-for-nostr' ); ?>
				</button>
				<span id="synalysis-blog-importer-login-status" class="description" style="display:block;margin-top:0.5em;" aria-live="polite"></span>
			</p>
		</div>
		<?php
	}
	...
	public function rest_login_challenge( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		if ( ! $this->is_enabled() ) {
			return new \WP_Error( 'nostr_login_disabled', __( 'Extension sign-in is not available.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 403 ) );
		}
		...
		if ( ! wp_verify_nonce( $nonce, self::NONCE_ACTION ) ) {
			return new \WP_Error( 'invalid_nonce', __( 'Invalid or expired security token. Reload the login page.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 403 ) );
		}

		$pubkey = isset( $params['pubkey'] ) ? strtolower( trim( (string) $params['pubkey'] ) ) : '';
		if ( strlen( $pubkey ) !== 64 || ! ctype_xdigit( $pubkey ) ) {
			return new \WP_Error( 'invalid_pubkey', __( 'Invalid public key.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 400 ) );
		}

		$authors = array_flip( nostr_wp_blog_get_author_pubkey_hexes() );
		if ( ! isset( $authors[ $pubkey ] ) ) {
			return new \WP_Error( 'not_author', __( 'This key is not listed as an author for this site.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 403 ) );
		}
	...
	public function rest_login_verify( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( strlen( $token ) !== 32 ) {
			return new \WP_Error( 'invalid_token', __( 'Invalid session.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 400 ) );
		}

		$data = get_transient( self::TRANSIENT_PREFIX . $token );

		if ( ! is_array( $data ) || ! isset( $data['pubkey'], $data['challenge'], $data['exp'] ) ) {
			return new \WP_Error( 'expired', __( 'Sign-in session expired. Try again.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 410 ) );
		}
		...
		if ( ! is_array( $event_raw ) ) {
			return new \WP_Error( 'invalid_event', __( 'Invalid signed event.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 400 ) );
		}
		...
		if ( ! $verified->verify( $event_obj ) ) {
			return new \WP_Error( 'bad_sig', __( 'Signature verification failed.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 403 ) );
		}

		if ( $event->kind !== 22242 ) {
			return new \WP_Error( 'wrong_kind', __( 'Wrong event kind.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 400 ) );
		}

		$pk = strtolower( (string) $event->pubkey );
		if ( $pk !== (string) $data['pubkey'] ) {
			return new \WP_Error( 'pubkey_mismatch', __( 'Public key mismatch.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 403 ) );
		}
		...
		if ( ! hash_equals( (string) $data['challenge'], $chal ) ) {
			return new \WP_Error( 'challenge_mismatch', __( 'Challenge mismatch.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 403 ) );
		}

		$relay = $this->tag_value( $event->tags, 'relay' );
		if ( $relay === '' || ! hash_equals( $this->relay_uri(), $relay ) ) {
			return new \WP_Error( 'relay_mismatch', __( 'Relay tag mismatch.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 403 ) );
		}

		$skew = abs( time() - (int) $event->created_at );
		if ( $skew > 900 ) {
			return new \WP_Error( 'stale', __( 'Event timestamp is too far from server time.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 400 ) );
		}

		$uid = (int) nostr_wp_blog_get_settings()['nostr_login_user_id'];
		$user = get_userdata( $uid );
		if ( ! $user ) {
			return new \WP_Error( 'no_user', __( 'Extension sign-in is not configured.', 'synalysis-blog-importer-for-nostr' ), array( 'status' => 500 ) );
		}
