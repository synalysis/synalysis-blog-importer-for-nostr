...
final class Nostr_WP_Blog_Sync {
	...
	public function sync( bool $manual ): array {
		...
		if ( $authors === array() ) {
			$result['errors'][] = __( 'No author npubs or pubkeys configured.', 'nostr-wp-blog' );
			$this->persist_sync_meta( $result, $manual );
			return $result;
		}

		if ( $relays === array() ) {
			$result['errors'][] = __( 'No relay URLs configured.', 'nostr-wp-blog' );
			$this->persist_sync_meta( $result, $manual );
			return $result;
		}
		...
		foreach ( $relays as $relay_url ) {
			try {
				...
				$result['errors'][] = sprintf(
					...
					__( 'Relay %1$s: %2$s', 'nostr-wp-blog' ),
					$relay_url,
					$e->getMessage()
				);
			}
